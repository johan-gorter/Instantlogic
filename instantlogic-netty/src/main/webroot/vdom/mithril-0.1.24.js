Mithril = m = new function app(window, undefined) {
    ...
    if(dataType == sArr) {
        ...
        for(var i = 0; i < data.length; i++) {
          if(data[i] && data[i].attrs) {
            if(data[i].attrs.key != null) {
              ...
              unkeyed.push({ index: i, element: parentElement.childNodes[i] || $document.createElement("div") });
            }
          }
        }
      ...
      for(var i = 0, cacheCount = 0; i < data.length; i++) {
        ...
        if(item.$trusted) {
          ...
          subArrayCount += type.call(item) == sArr ? item.length : 1;
        }
      ...
      if(data.tag != cached.tag || dataAttrKeys.join() != Object.keys(cached.attrs).join() || data.attrs.id != cached.attrs.id) {
        ...
        if(cached.configContext && typeof cached.configContext.onunload == sFn) {
          cached.configContext.onunload();
        }
      }
      if(type.call(data.tag) != sStr) {
        return;
      }
  ...
  m.route = function () {
    ...
    //m.route(route)
      ...
      if(querystring) {
        currentRoute += (currentRoute.indexOf("?") === -1 ? "?" : "&") + querystring;
      }
