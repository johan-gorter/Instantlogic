...
public class GsonInstanceAdapter implements JsonSerializer<Instance>, JsonDeserializer<Instance> {
			...
			if (!attribute.isReadOnly()) {
				...
				if (attributeValue.hasStoredValue()) {
					if (attribute.isMultivalue()) {
						WriteableAttributeValue attributeValues = (WriteableAttributeValue) attributeValue;
			...
			if (!relation.isReadOnly()) {
				...
				if (attributeValue.hasStoredValue()) {
					if (relation.isMultivalue()) {
						WriteableAttributeValue target = (WriteableAttributeValue) attributeValue;
						JsonArray values = new JsonArray();
		...
		for (Relation relation : entity.getRelations()) {
			if (!relation.isReadOnly()) {
				if (data.has(relation.getName())) {
					...
					if (relation.isOwner()) {
						if (relation.isMultivalue()) {
							setMultivalueRelation(relation, (WriteableAttributeValue) relationValue, value, result, caseInstance, instances);
						} else {
			...
			if (!relation.isReadOnly()) {
				if (data.has(relation.getName())) {
					...
					if (!relation.isOwner()) {
						...
						if (relation.isMultivalue()) {
							WriteableAttributeValue values = (WriteableAttributeValue) relationValue;
							for (JsonElement item : ((JsonArray) value)) {
								if (item instanceof JsonObject) {
									// probably a static instance
									values.setOrAdd(deserializeStaticInstance((JsonObject)item, result.getMetadata().getCaseAdministration()));
								} else {
									String itemId = item.getAsString(); // Id of another instance
									values.setOrAdd(instances.get(itemId));
								}
							}
						} else {
	...
	private void setMultivalueRelation(Relation relation, WriteableAttributeValue relationValue, JsonElement value, Instance owner, Instance caseInstance, Map<String, Instance> instances) {
	...
	private void setMultivalueAttribute(Attribute attribute, WriteableAttributeValue attributeValue, JsonElement value, CaseAdministration caseAdministration) {
		JsonArray values = (JsonArray) value;
		for (JsonElement item : values) {
			attributeValue.setOrAdd(toPrimitive(attribute, item, caseAdministration));
		}
	}
